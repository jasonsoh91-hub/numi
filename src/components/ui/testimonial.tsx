...
export const AnimatedTestimonials = ({
  ...
  // Deterministic rotation per index — avoids SSR/client hydration mismatch.
  const rotateFor = (index: number) => `${((index * 37) % 16) - 8}deg`;
              ...
              <AnimatePresence>
                {testimonials.map((testimonial, index) => (
                  <motion.div
                    ...
                    initial={{ opacity: 0, scale: 0.9, y: 50, rotate: rotateFor(index) }}
                    animate={{
                      opacity: isActive(index) ? 1 : 0.5,
                      scale: isActive(index) ? 1 : 0.9,
                      y: isActive(index) ? 0 : 20,
                      zIndex: isActive(index) ? testimonials.length : testimonials.length - Math.abs(index - active),
                      rotate: isActive(index) ? '0deg' : rotateFor(index),
                    }}
